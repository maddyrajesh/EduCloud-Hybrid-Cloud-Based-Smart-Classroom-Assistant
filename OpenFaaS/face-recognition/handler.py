...
def handle(req):
    """Handle function for OpenFaaS.

    Args:
        req (str): JSON string containing the bucket and key information.
    """
    data = json.loads(req)
    ...
    if not bucket or not key:
        return json.dumps({"statusCode": 400, "body": "No bucket or key provided."})

    try:
        response = face_recognition_handler(bucket, key)
        return json.dumps({"statusCode": 200, "body": response})
    except Exception as e:
        logging.error(f"Error in face_recognition_handler: {e}", exc_info=True)
        return json.dumps({"statusCode": 500, "body": f"Error processing the video: {e}"})
